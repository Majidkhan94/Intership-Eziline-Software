...
export const Adminregistration = () => {
  ...
  const submitHandle = async (e)=>{
        ...
        try {
          ...
          const response = await axios.post("/api/v1/auth/adminregistration", userData);
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-6">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-100">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-800 tracking-tight">Admin Portal</h2>
        </div>
  ...
  <span className={`text-sm mt-2 font-semibold ${color}`}>
    {message}
        ...
        <form className="space-y-5" onSubmit={submitHandle}>
          ...
          <div>
            <input type="text" placeholder="Username" className="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200" onChange={(item)=>setusername(item.target.value)}/>
          </div>
