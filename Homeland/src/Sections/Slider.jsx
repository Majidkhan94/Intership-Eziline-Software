...
export const Slider = () => {
  ...
  return (
    ...
    <section className="relative top-0 left-0 w-screen h-82 md:h-screen overflow-hidden">

      {/* <div className="overflow-hidden"> */}
        
        {/* SLIDER IMAGES */}
        {Slide.map((item, idx) => (
          <img key={idx} src={item} className={`absolute top-0 left-0 w-full h-100 md:h-full object-cover transition-opacity duration-700 ${ idx === current ? "opacity-100 z-10" : "opacity-0 z-0" }`} /> ))}

        {/* DATA OVERLAY (MIDDLE) */}
        {overlayVisible && (
          <div className="absolute w-full h-full z-20 flex items-center justify-center text-white text-center flex-col">
...
<div className="absolute w-full h-full z-10 flex items-center justify-between px-4 md:px-8">
  <button
    onClick={() =>
